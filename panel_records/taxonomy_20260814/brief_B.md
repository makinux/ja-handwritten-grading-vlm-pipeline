# 設計判断ブリーフ: 誤り種別タキソノミの評価粒度(パネリスト B 用)
...
## 問い(選択肢は与えない——あなたが設計せよ)

この実測を踏まえ、**誤り種別タキソノミの評価粒度と KPI をどう扱うべきか**。取り得る設計選択肢の洗い出しから自分で行い、推奨を一つに絞れ。「よくある折衷」に逃げず、判断軸(教育的価値/観測可能性/KPI の誠実性/実装コスト/sim2real への影響)の間のトレードオフを明示的に裁くこと。
...
- 少なくとも 1 つ、与えられた前提や本ブリーフの枠組み自体に明確に反対する論点を含めること
